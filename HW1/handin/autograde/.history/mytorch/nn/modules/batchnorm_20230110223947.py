...
class BatchNorm1d:
    ...
    def backward(self, dLdBZ):
        ...
        dLdV        = -1/2 * (dLdNZ * (self.Z - self.M) * (self.V + self.eps)**(-3/2)).sum(axis = 0, keepdims = True) # TODO
        dLdM        = -(dLdBZ * (self.V + self.eps)**(-1/2) ).sum(axis = 0, keepdims = True) - \
            2/self.N * dLdV * (self.Z - self.M).sum(axis = 0, keepdims = True) # TODO
        
        dLdZ        = dLdNZ * (self.V + self.eps) ** (-1/2)+ dLdV * 2/self.N * (self.Z - self.M) + \
         1 / self.N * dLdM  # TODO
