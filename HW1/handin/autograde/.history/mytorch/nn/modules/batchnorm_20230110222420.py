...
class BatchNorm1d:
    ...
    def backward(self, dLdBZ):
        ...
        dLdV        = -0.5 * (dLdNZ * (self.Z - self.M) * (self.V + self.eps)**(3/2)).sum(axis = 0, keepdims = True) # TODO
        dLdM        = (dLdBZ * (self.V + self.eps)**(1/2) ).sum(axis = 0, keepdims = True) - \
            1/2 * ( dLdNZ * (self.Z - self.M) * (self.V + self.eps)**(3/2)  * (-2/self.N * (self.Z - self.M).sum() )).sum(axis = 0, keepdims = True) # TODO
